...
#p3
#date 22年10月18日 周二 23:01

## 站点支持夜间模式

在 Win 10 本上玩老旧游戏，之后用 FireFox 查了一下暗黑模式插件，找到了 [darkreader](https://darkreader.org/) 扩展，其实也支持作为 Chrome 扩展，于是试用了一下，感觉完成度很高，阔以。

但是更没想到的是，因为是开源软件，github 介绍页上还包含了一个 `darkreader.min.js` 可以用在网站上，使用起来也比较简单，所以，很快，半小时不到，本站就支持夜间模式了。

方法是点击右边的 __Function__ 下面的 <a onclick="changeTheme()">theme ➟</a>，就可以切换日间、夜间模式了。

跟用 FireFox/Chrome 插件效果一样的。

#category ThisSite

#p2
#date 22年10月15日 周六 23:17

## 使用 Pagefind 支持静态网页站内搜索

在微博上看到了人家介绍 [Pagefind](https://pagefind.app/)，一个静态网站搜索工具，试用了一下，感觉很不错，部署也很方便。

这个工具的可以在每次 html 静态内容生成之后，对 html 网页进行分析，可以指定语言等，生成的是一个个独立的 zip 文件，
以及 css 和 js，全部生成在某个目录下，不需要其他的打包工具了。

打包的命令行比如，最后加的 `--serve` 参数可以拉起一个本地的 http 服务器，可以浏览器直接预览。

```bash
$ pagefind --bundle-dir ./pagefind --source ./publish --verbose --serve
```

运行的时候，先拉起一个 html，指定一个 id 内容节点，然后运行 js 提供的类进行 UI 渲染，它会监听 input 的输入，
然后动态更新节点下面搜索到的内容。
...
很好操作，UI 就不放上来了，工具是 Rust 搞的，要赞一个。
