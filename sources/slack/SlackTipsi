...
假设你最喜爱的编辑器是 mcedit。让我们用它打开 ~/.bashrc 并把这些置于alias 后[1](#fn1)：
```source  
...
这里我使用 edb 作为编辑 ~/.Bashrc[2](#fn2) 的缩写。
...
就像我们所看到的，别名和函数 是强大的工具。他们可以去作许许多多不同的事：
 ...
 - 目录导航：
```source
     alias cda='cd ~/some/directory/where../writing/an/Article'
```    
...
```  
 ...
 - 备份文件，比方说，重置文件：
```source  
     BACKUP_DIR="/pa../the/backup/directory"
     alias back="cd $BACKUP_DIR && tgz conf-`date +%F`.tgz ~/.??* && \
       ls && cd"
```    

 - 保存升级和移除包的记录（当然，这些是为 root 的 .bashrc[3](#fn3) 准备的）：
```source
...
```    
 ...
 - 看搜集的 jpg 图片：
```source  
     alias slide='qiv -f -s --delay=5 -i ./*.jpg'
```    
...
[2](#fn2) 可以随便选择其他的名字。但在创建前得检查一下它是否已经被系统的shell 或者你 PATH 变量里的程序占用。为此，你可以运行：
```source  
   $ help new_name
   $ which new_name
```  
...
[3](#fn3) 特别的，这些记录可以被用来检查这个或那个包在被移除或升级之后是否留下了一些没有被删除的文件。
